...
npm run deploy:prod  # Deploy to Vercel (vercel --prod --force)
```

## Architecture Overview

This is a personal portfolio website built with Next.js 15 (App Router) and React 19 RC.

### Feature-Based Structure

The codebase uses a feature-based architecture under `src/app/features/`:

- **header/** - Hero section with profile
- **about/** - About section
- **skill/** - Skills grid with custom SVG icons (icons in `skill/Icons/`)
- **experience/** - Work experience timeline
- **work/** - Portfolio projects with Notion integration
- **contact/** - Email contact form (currently disabled)
- **navigation/** - Section navigation controls
- **notion/** - Notion API integration for dynamic content

Shared utilities live in `src/app/shared/`:
- `components/` - Motion-wrapped MUI components (MotionStack, MotionCard, etc.)
- `states/PopupId/` - Zustand store for popup state management

### API Routes

API routes are in `pages/api/` (Pages Router pattern):
- `contact.ts` - POST endpoint for email via Nodemailer
- `notionPage.ts` - GET proxy for Notion API

### Key Technologies

- **Styling**: Material-UI v6 with Emotion CSS-in-JS
- **Animations**: Framer Motion (components wrapped as Motion* in shared/components)
- **State**: Zustand for UI state, TanStack React Query for server-state
- **Content**: Notion client libraries (react-notion-x) for portfolio content
...
- Gmail SMTP credentials for contact form (see `.env.development` for structure)
- Notion API credentials for portfolio content

## Deployment

- **Platform**: Vercel
- **Domain**: chocodigo.com (Gabia)
- **Production URL**: https://www.chocodigo.com
